...
class PublicEndpointPolicyTest {
    ...
    void allowlistIsMethodAwareAndDoesNotExposeNestedBusinessEndpoints() {
        ...
        assertThat(policy.isPublic(HttpMethod.POST, "/api/v1/auth/forgot-password")).isTrue();
        assertThat(policy.isPublic(HttpMethod.POST, "/api/v1/auth/reset-password")).isTrue();
        assertThat(policy.isPublic(HttpMethod.POST, "/api/v1/auth/verify-email")).isTrue();
        assertThat(policy.isPublic(HttpMethod.POST, "/api/v1/auth/resend-verification")).isTrue();
        assertThat(policy.isPublic(HttpMethod.GET, "/api/v1/auth/dev/action-token")).isTrue();
        ...
        assertThat(policy.isPublic(HttpMethod.POST, "/api/v1/auth/dev/action-token")).isFalse();
