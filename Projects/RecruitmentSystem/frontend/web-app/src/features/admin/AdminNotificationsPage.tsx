...
export function AdminNotificationsPage() {
  ...
  return <main className="admin-page">
    <header className="admin-page__hero"><div><span>Notification operations</span><h1>Thông báo hệ thống</h1><p>Tạo thông báo cho một user hoặc broadcast theo đúng contract Notification Service.</p></div><BellRing /></header>
    {feedback && <p className="admin-feedback" role="status">{feedback}</p>}
    <section className="admin-dashboard-grid">
      <article className="admin-panel"><form className="admin-form" onSubmit={personal.handleSubmit((values) => create.mutate({ ...values, relatedResourceType: values.relatedResourceType || undefined, relatedResourceId: values.relatedResourceId || undefined }))}><div><span className="admin-badge">PERSONAL</span><h2>Thông báo cá nhân</h2></div><label>Recipient user UUID<input {...personal.register('recipientUserId')} />{personal.formState.errors.recipientUserId && <small>{personal.formState.errors.recipientUserId.message}</small>}</label><label>Event type<select {...personal.register('eventType')}>{events.map((event) => <option key={event}>{event}</option>)}</select></label><label>Tiêu đề<input {...personal.register('title')} />{personal.formState.errors.title && <small>{personal.formState.errors.title.message}</small>}</label><label>Nội dung<textarea {...personal.register('content')} />{personal.formState.errors.content && <small>{personal.formState.errors.content.message}</small>}</label><div className="admin-form__grid"><label>Resource type<input {...personal.register('relatedResourceType')} /></label><label>Resource UUID<input {...personal.register('relatedResourceId')} />{personal.formState.errors.relatedResourceId && <small>{personal.formState.errors.relatedResourceId.message}</small>}</label></div>{create.isError && <p className="admin-feedback admin-feedback--error">{getErrorMessage(create.error)}</p>}<button className="admin-button" disabled={create.isPending}><Send />{create.isPending ? 'Đang gửi…' : 'Gửi cá nhân'}</button></form></article>
      <article className="admin-panel"><form className="admin-form" onSubmit={broadcast.handleSubmit((values) => sendBroadcast.mutate(values))}><div><span className="admin-badge">ALL USERS</span><h2>Broadcast</h2><p>Backend cố định event type là SYSTEM_ANNOUNCEMENT.</p></div><label>Tiêu đề<input {...broadcast.register('title')} />{broadcast.formState.errors.title && <small>{broadcast.formState.errors.title.message}</small>}</label><label>Nội dung<textarea {...broadcast.register('content')} />{broadcast.formState.errors.content && <small>{broadcast.formState.errors.content.message}</small>}</label>{sendBroadcast.isError && <p className="admin-feedback admin-feedback--error">{getErrorMessage(sendBroadcast.error)}</p>}<button className="admin-button" disabled={sendBroadcast.isPending}><Radio />{sendBroadcast.isPending ? 'Đang phát…' : 'Phát broadcast'}</button></form></article>
    </section>
    <section className="admin-toolbar"><label>Recipient UUID<input value={recipient} onChange={(e) => { setRecipient(e.target.value.trim()); setPage(0) }} placeholder="Để trống: thông báo của Admin" /></label><label>Event<select value={eventType} onChange={(e) => { setEventType(e.target.value as NotificationEventType | ''); setPage(0) }}><option value="">Tất cả</option>{events.map((event) => <option key={event}>{event}</option>)}</select></label><label>Từ khóa<input value={query} onChange={(e) => { setQuery(e.target.value); setPage(0) }} /></label></section>
