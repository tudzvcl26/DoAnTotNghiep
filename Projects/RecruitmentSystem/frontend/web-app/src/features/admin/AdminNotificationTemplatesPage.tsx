...
import { notificationChannelLabels, notificationEventLabels } from './admin.labels'
...
export function AdminNotificationTemplatesPage() {
  ...
  return <main className="admin-page"><header className="admin-page__hero"><div><span>Vận hành thông báo</span><h1>Mẫu thông báo</h1><p>Tạo, cập nhật và bật hoặc tắt mẫu. Dịch vụ không hỗ trợ xóa nên giao diện không cung cấp thao tác này.</p></div><LayoutTemplate /></header>
    ...
    {open && <section className="admin-panel"><form className="admin-form" onSubmit={form.handleSubmit((values) => save.mutate(values))}><div className="admin-form__grid"><label>Mã mẫu<input {...form.register('code')} disabled={Boolean(editing)} />{form.formState.errors.code && <small>{form.formState.errors.code.message}</small>}</label><label>Loại sự kiện<select {...form.register('eventType')}>{events.map((event) => <option key={event} value={event}>{notificationEventLabels[event]}</option>)}</select></label><label>Kênh<select {...form.register('channel')}>{channels.map((channel) => <option key={channel} value={channel}>{notificationChannelLabels[channel]}</option>)}</select></label><label>Tiêu đề mẫu<input {...form.register('titleTemplate')} />{form.formState.errors.titleTemplate && <small>{form.formState.errors.titleTemplate.message}</small>}</label><label className="admin-form__wide">Nội dung mẫu<textarea {...form.register('contentTemplate')} />{form.formState.errors.contentTemplate && <small>{form.formState.errors.contentTemplate.message}</small>}</label></div>{save.isError && <p className="admin-feedback admin-feedback--error">{getErrorMessage(save.error)}</p>}<div className="admin-form__actions"><button className="admin-button admin-button--secondary" type="button" onClick={() => setOpen(false)}>Hủy</button><button className="admin-button" disabled={save.isPending}>{save.isPending ? 'Đang lưu…' : editing ? 'Lưu thay đổi' : 'Tạo mẫu'}</button></div></form></section>}
