...
export function CvTemplatesPage() {
  ...
  return <main className="cv-page cv-template-page">
    ...
    <section className="cv-template-grid">
      {visible.map((template) => <article className="cv-template-card" key={template.id}><Link className="cv-template-card__preview" to={`/cv/templates/${template.id}`} aria-label={`Xem trước mẫu ${template.name}`}><CvPreview compact content={sampleCv} templateId={template.id} /><span><Eye /> Xem mẫu</span></Link><div className="cv-template-card__content"><span>{template.style}</span><h2>{template.name}</h2><p>{template.description}</p><small>Phù hợp: {template.bestFor}</small><div className="cv-template-card__actions"><Link className="cv-button" to={`/cv/templates/${template.id}`}><Eye /> Xem trước</Link><Link className="cv-button cv-button--primary" to={`/cv/templates/${template.id}`}>Chọn mẫu</Link></div></div></article>)}
    </section>
