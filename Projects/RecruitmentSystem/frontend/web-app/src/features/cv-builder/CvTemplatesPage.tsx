...
const styles = ['Tất cả', ...cvTemplateCategories] as const
...
export function CvTemplatesPage() {
  ...
  return <main className="cv-page cv-template-page">
    <header className="cv-page__hero"><div><span><Sparkles /> {cvTemplates.length} thiết kế gốc · 6 nhóm phong cách</span><h1>Mẫu CV</h1><p>Chọn phong cách phù hợp. Một bộ dữ liệu có thể đổi mẫu bất cứ lúc nào.</p></div><Link className="cv-button" to="/cv">CV của tôi</Link></header>
    ...
    <section className="cv-template-grid">
      {visible.map((template) => <article className="cv-template-card" key={template.id}><Link className="cv-template-card__preview" to={`/cv/templates/${template.id}`} aria-label={`Xem trước mẫu ${template.name}`}><CvPreview compact content={applyCvTemplate(sampleCv, template.id)} templateId={template.id} /><span><Eye /> Xem mẫu</span></Link><div className="cv-template-card__content"><span>{template.style}</span><h2>{template.name}</h2><p>{template.description}</p><small>Phù hợp: {template.bestFor}</small><div className="cv-template-card__actions"><Link className="cv-button" to={`/cv/templates/${template.id}`}><Eye /> Xem trước</Link><Link className="cv-button cv-button--primary" to={`/cv/new?template=${template.id}&source=blank`}>Tạo CV trống</Link></div></div></article>)}
    </section>
    {visible.length === 0 && <p className="cv-state" role="status">Không tìm thấy mẫu phù hợp. Hãy đổi từ khóa hoặc nhóm phong cách.</p>}
