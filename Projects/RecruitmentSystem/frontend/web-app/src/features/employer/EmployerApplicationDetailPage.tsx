...
export function EmployerApplicationDetailPage() {
  ...
  return <main className="employer-application-detail">
    ...
    <header><div><span className={`employer-application-status employer-application-status--${data.status.toLowerCase()}`}>{employerApplicationStatusLabels[data.status]}</span><h1>{jobTitle}</h1><p>{candidate?.displayName || `Ứng viên ${data.candidateId}`} · Nộp {formatEmployerApplicationDate(data.appliedAt)}</p></div><FileUser /></header>
