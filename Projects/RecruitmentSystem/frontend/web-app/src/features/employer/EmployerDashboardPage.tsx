...
export function EmployerDashboardPage() {
  ...
  return <main className="employer-dashboard">
    ...
    <section className="employer-metrics" aria-label="Tổng quan tuyển dụng">
      ...
      <article><span><BriefcaseBusiness /></span><div><small>Tổng việc làm</small><strong>{jobStatistics.isPending ? '—' : jobStatistics.data?.total ?? 0}</strong></div></article>
      <article><span><UsersRound /></span><div><small>Ứng tuyển toàn doanh nghiệp</small><strong>{applicationStatistics.isPending ? '—' : applicationStatistics.data?.total ?? 0}</strong></div></article>
