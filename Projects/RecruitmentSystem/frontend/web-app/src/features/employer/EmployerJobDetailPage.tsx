...
import { AlertCircle, ArrowLeft, BriefcaseBusiness, CalendarDays, CheckCircle2, Edit3, RadioTower, RefreshCw, ShieldAlert, UsersRound, XCircle } from 'lucide-react'
...
import {
  closeEmployerJob, employerCompanyKey, employerJobKey, getEmployerCompanies, getEmployerJob, publishEmployerJob,
} from './employer.api'
...
export function EmployerJobDetailPage() {
  ...
  const actionError = publish.error ?? close.error
  return <main className="employer-job-detail">
    ...
    <header><div><span className={`employer-job-status employer-job-status--${job.data.status.toLowerCase()}`}>{statusLabels[job.data.status]}</span><h1>{job.data.title}</h1><p>{ownedCompany.name} · {job.data.jobCode}</p></div><div><ButtonLink to={`/employer/jobs/${jobId}/edit`} variant="secondary"><Edit3 /> Chỉnh sửa</ButtonLink>{job.data.status === 'DRAFT' && <Button type="button" disabled={publish.isPending} onClick={() => { if (window.confirm('Đăng tuyển việc làm này? Nội dung sẽ xuất hiện công khai.')) publish.mutate() }}><CheckCircle2 /> {publish.isPending ? 'Đang đăng...' : 'Đăng tuyển'}</Button>}{job.data.status === 'PUBLISHED' && <Button type="button" variant="dark" disabled={close.isPending} onClick={() => { if (window.confirm('Đóng việc làm này? Tin sẽ không còn hiển thị công khai.')) close.mutate() }}><XCircle /> {close.isPending ? 'Đang đóng...' : 'Đóng tin'}</Button>}</div></header>
