...
export function EmployerCompanyForm({ company, pending, error, onCancel, onSubmit }: {
  ...
  return <form className="employer-company-form" onSubmit={handleSubmit((values) => onSubmit(company ? updateValues(values) : withoutEmptyValues(values)))} noValidate>
    ...
    <div className="employer-company-form__grid">
      ...
      <label>Loại hình<select {...register('companyType')}><option value="">Chưa xác định</option>{COMPANY_TYPES.map((value) => <option key={value} value={value}>{typeLabels[value]}</option>)}</select></label>
      <label>Quy mô<select {...register('companySize')}><option value="">Chưa xác định</option>{COMPANY_SIZES.map((value) => <option key={value} value={value}>{sizeLabels[value]}</option>)}</select></label>
